...
if selected == "Home":
    ...
    model_path = os.path.join(CHKP_DIR, "fspen_chkp", "TrainConfig48kHzEnc2x_ver2_real#1.pt")
    ...
    with torch.no_grad():
        if input_files:
            ...
            if len(input_files) > 1:
                st.warning(f"You uploaded {len(input_files)} files. Please upload no more than {1}.")
            else:
                if st.button("Start Prediction"):
                    ...
                    for ind, uploaded_file in enumerate(input_files):
                        ...
                        try:
                            ...
                            tGrid = np.arange(0,spec_vis.shape[2]*HOP_LENGTH, HOP_LENGTH)/SR
                            fGrid = np.arange(0,N_FFTS/2+0.00001)/(N_FFTS)*SR
                            tt,ff = np.meshgrid(tGrid,fGrid)

                            fig, ax = plt.subplots(figsize=(10, 4))

                            img=ax.pcolormesh(tt,ff,20*torch.log10(spec_vis.squeeze()+1e-8),cmap="gist_heat")
