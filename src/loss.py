...
def _compute_mr(
    Y: torch.Tensor, Y_abs: torch.Tensor, S: torch.Tensor, S_abs: torch.Tensor
) -> torch.Tensor:
    x1 = F.mse_loss(Y_abs, S_abs)
    x2 = F.mse_loss(
        torch.abs(Y_abs * (Y / (torch.abs(Y) + 1e-9))),
        torch.abs(S_abs * (S / (torch.abs(S) + 1e-9))),
    )
    return x1 + x2
...
def hybrid_loss(pred_stft, true_stft, n_fft):
    ...
    y_pred = torch.istft(
        ...
        n_fft // 2,
        n_fft,
        window=vorbis_window(n_fft).to(pred_stft.device),
    )
    y_true = torch.istft(
...
def loss_compressed_MR(
    ...
    for nfft in nffts:
        Y, S, Y_abs, S_abs = _prepare_mr_terms(input, target, nfft, hop_fr, gamma)

        loss += (1 - lambd) * torch.mean(
            (Y_abs - S_abs).abs() ** 2
        ) + lambd * torch.mean(
            torch.abs(
                Y_abs * (Y / (torch.abs(Y) + 1e-9))
                - S_abs * (S / (torch.abs(S) + 1e-9))
            )
            ** 2
        )
...
def loss_ri_mag(
    ...
    for nfft in nffts:
        ...
        loss_2 = F.l1_loss(
            torch.view_as_complex(X).abs(), torch.view_as_complex(Y).abs()
        )
...
def loss_MR_w(input: torch.Tensor, target: torch.Tensor, lens: list = None):
    ...
    for seg in lens:
        ...
        assert len(input_chunks) == len(
            target_chunks
        ), f"{len(input_chunks)} != {len(target_chunks)}"
        loss_interm = torch.zeros(
            (input.shape[0],), device=input.device, dtype=input.dtype
        )
...
def loss_tot(
    ...
    target_noise: torch.Tensor = None,
    rir: torch.Tensor = None,
    target_rir: torch.Tensor = None,
    gamma: float = 0.3,
    nffts: list = None,
    hop_fr: float = 0.75,
    lens: list = None,
...
if __name__ == "__main__":
    ...
    h0 = [[torch.randn(1, 2 * 32, 16 // 8) for _ in range(8)] for _ in range(3)]
