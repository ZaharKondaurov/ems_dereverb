...
class FullBandEncoder(nn.Module):
    def __init__(self, configs: TrainConfig):
        ...
        for encoder_name, conv_parameter in configs.full_band_encoder.items():
            self.full_band_encoder.append(
                FullBandEncoderBlock(**conv_parameter, act=configs.mag_act["full"])
            )
    ...
    def forward(self, complex_spectrum: Tensor):
        ...
        for i, encoder in enumerate(self.full_band_encoder):
            ...
            assert (
                torch.isnan(complex_spectrum).any().item() is False
            ), f"fullband enc conv_{i} out has NaNs"
            full_band_encodes.append(complex_spectrum)

        global_feature = self.global_features(complex_spectrum)
        assert (
            torch.isnan(global_feature).any().item() is False
        ), f"fullband global_feature out has NaNs"
        ...
        for ind, (decoder_name, parameters) in enumerate(fbd_items):
            ...
            self.full_band_decoders.append(
                FullBandDecoderBlock(
                    **parameters, split_act=split_act, mag_act=mag_act, pha_act=pha_act
                )
            )

    def forward(self, feature: Tensor, encode_outs: list):
        for i, (decoder, encode_out) in enumerate(
            zip(self.full_band_decoders, encode_outs)
        ):
            feature = decoder(feature, encode_out)
            assert (
                torch.isnan(feature).any().item() is False
            ), f"fullband dec conv_{i} out has NaNs"
