    ...
    def __init__(self, configs: TrainConfig):
        ...
        for encoder_name, conv_parameters in configs.sub_band_encoder.items():
            self.sub_band_encoders.append(
                SubBandEncoderBlock(
                    **conv_parameters["conv"], mag_act=configs.mag_act["sub"]
                )
            )
    ...
    def __init__(self, configs: TrainConfig):
        ...
        for encoder_name, layer_parameters in configs.sub_band_encoder.items():
            ...
            for conv_parameters in layer_parameters["convs"]:
                sub_band_layer.append(
                    FullBandEncoderBlock(
                        **conv_parameters, normalize=False, act=configs.mag_act["sub"]
                    )
                )
    ...
    def forward(self, amplitude_spectrum: Tensor):
        ...
        for ind, encoder in enumerate(self.sub_band_encoders):
            ...
            for i, conv in enumerate(encoder):
                ...
                assert (
                    torch.isnan(encode_out).any().item() is False
                ), f"subband enc conv {ind} group {i} layer out has NaNs"
        ...
        local_feature = torch.cat(
            [outs[-1] for outs in sub_band_encodes], dim=2
        )  # feature cat
...
class SubBandDecoder_ext(nn.Module):
    def __init__(self, configs: TrainConfig):
        ...
        for i, (decoder_name, layer_parameters) in enumerate(sbd_items):
            self.bands.append(
                {"start": start_idx, "end": start_idx + layer_parameters["width"]}
            )
    ...
    def forward(self, feature: Tensor, sub_encodes: list):
        sub_decoder_outs = []
        for idx, (decoder, sub_encode) in enumerate(
            zip(self.sub_band_decoders, sub_encodes)
        ):
            ...
            for i, conv in enumerate(decoder):
                ...
                assert (
                    torch.isnan(sub_decoder_out).any().item() is False
                ), f"subband dec conv {idx} group {i} layer out has NaNs"
                decode_in = sub_decoder_out

            first_dim, bands, band_width = sub_decoder_out.shape
            sub_decoder_out = torch.reshape(
                sub_decoder_out, shape=(first_dim, bands * band_width)
            )
