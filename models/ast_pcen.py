...
class _PCEN(nn.Module):
    ...
    def forward(
        ...
        alpha = torch.min(self.alpha, torch.tensor(1.0)).item()
        root = torch.max(self.root, torch.tensor(1.0)).item()
        one_over_root = 1.0 / root
        ema_smoother, hidden_state = self.ema(mag_spec, initial_state)
        # print(alpha, ema_smoother.isnan().any(), ' alpha, ema_smoother')
        mag_spec = mag_spec.transpose(1, -1)
        ema_smoother = ema_smoother.transpose(1, -1)
        # Equation (1) in [1]
        # print(alpha, self.delta, root)
        # print(mag_spec.isnan().any(), self.floor, alpha, self.delta, one_over_root)
        out = (
            mag_spec / (self.floor + ema_smoother) ** alpha + self.delta
        ) ** one_over_root - self.delta ** one_over_root
        # print(out.isnan().any(), ' pcen out')
...
#     audio, sr = soundfile.read('/Users/i.beskrovnyy/examples/fav_file/noisy16/003c3dce-3c21-4b12-b1e5-6e4f4d398bed.wav')
#     spec = torch.stft(torch.as_tensor(audio), 1024, 512, 1024, torch.hann_window(1024), return_complex=False).unsqueeze(0)
#     spec = spec[..., 0]
#     print(type(spec), spec.shape)
#     pcen = PCEN()
#     pcen_spec = pcen(spec.abs())
#     print(type(pcen_spec), pcen_spec.shape)
#     print(pcen_spec.isnan().any())
#
#     fig, (ax1, ax2) = plt.subplots(2,1)
#     img = librosa.display.specshow(librosa.amplitude_to_db(np.abs(spec.squeeze(0).numpy())), y_axis='log', x_axis='time', sr = sr, ax = ax1)
#     im2 = librosa.display.specshow(librosa.amplitude_to_db(np.abs(pcen_spec.squeeze(0).numpy())), y_axis='log', x_axis='time', sr = sr, ax = ax2)
#     plt.show()
