...
from models.subband_enc_dec import (
    SubBandEncoder,
    SubBandDecoder,
    SubBandEncoder_ext,
    SubBandDecoder_ext,
...
class FullSubPathExtension(nn.Module):
    def __init__(
        self,
        configs: TrainConfig,
        full_band_encoder: nn.Module = FullBandEncoder,
        sub_band_encoder: nn.Module = SubBandEncoder,
        full_band_decoder: nn.Module = FullBandDecoder,
        sub_band_decoder: nn.Module = SubBandDecoder,
        need_mask: bool = True,
        ...
        self.feature_merge_layer = nn.Sequential(
            nn.Linear(
                in_features=merge_channels, out_features=merge_channels // compress_rate
            ),
            nn.ELU(),
            nn.Conv1d(
        ...
        self.feature_split_layer = nn.Sequential(
            ...
            nn.Linear(
                in_features=merge_channels // compress_rate, out_features=merge_channels
            ),
            nn.ELU(),
        )
    ...
    def encode(self, in_complex_spectrum: Tensor, in_amplitude_spectrum: Tensor):
        ...
        merge_feature = torch.cat(
            tensors=[global_feature, local_feature], dim=2
        )  # feature cat
        ...
        merge_feature = torch.reshape(
            merge_feature, shape=(batch, frames, channels, frequency)
        )
    ...
    def decode(
        self,
        merge_feature: Tensor,
        full_band_encode_outs: Tensor,
        sub_band_encode_outs: Tensor,
    ):
        ...
        full_band_mask = self.full_band_decoder(
            split_feature[..., 0], full_band_encode_outs
        )

        sub_band_mask = self.sub_band_decoder(
            split_feature[..., 1], sub_band_encode_outs
        )
    ...
    def forward(
        self,
        in_complex_spectrum: Tensor,
        in_amplitude_spectrum: Tensor,
        hidden_state: list,
    ):
        """
        ...
        :return:
        """
        merge_feature, full_band_encode_outs, sub_band_encode_outs = self.encode(
            in_complex_spectrum, in_amplitude_spectrum
        )
        ...
        full_band_mask, sub_band_mask = self.decode(
            merge_feature, full_band_encode_outs, sub_band_encode_outs
        )
...
class FullSubPathExtension_ext(FullSubPathExtension):
    def __init__(
        self, configs: TrainConfig, need_mask: bool = True, print_mask: bool = False
    ):
        super().__init__(
            configs,
            full_band_encoder=FullBandEncoder,
            sub_band_encoder=SubBandEncoder_ext,
            full_band_decoder=FullBandDecoder,
            sub_band_decoder=SubBandDecoder_ext,
            need_mask=need_mask,
